...
// 			activeElements, concentration, concentration_temp, T);
        ...
        // if(flag && length > 2 * xStar0 && height > 2 * xStar0){
		if(flag && activeElements.size() >800){
            flag = false;
            /*
            ...
            */
            ...
            // std::cout << x.size() << " x " << xSize << std::endl;
            // std::cout << y.size() << " y " << ySize << std::endl;
            for(size_t i = 0; i < xSize; ++i){
                for(size_t j = 0; j < ySize; ++j){
                    mesh[i][j].setCoordinates(x[i], y[j]);
                }
            ...
            for(int j = 0; 2 * j < ySize - j00; ++j){
                for(int i = 0; 2 * i < xSize; ++i){
                    ...
                    openingNew[index[i][j00 + j]] = Wk[index[2 * i][j00 + 2 * j]];
                    openingNew[index[i][j00 - j]] = Wk[index[2 * i][j00 - 2 * j]];
                    // ai::printLine(ai::string(i) + ai::string("xx") + ai::string(j));
                    if(
                        ...
                        || (0 <= 2 * i <= j00 + 2 * j && RIBBON == mesh[2 * i][j00 + 2 * j].type)
                    ){
                    ...
                    // ai::printLine(ai::string(i) + ai::string("x") + ai::string(j));
                }
            }
            // ai::printLine("tut");
            for(size_t i = 0; i < xSize; ++i){
            ...
            for(std::size_t i = 0; i < ribbons.size(); ++i){
                mesh[ribbons[i].i][ribbons[i].j].type = RIBBON;
            }
            ...
            xStar0 *= 2;
            // ribbons = findRibbons(mesh, activeElements,
//                     distances, xStar0, dMax1, 2. * dx);
            std::cout << "Ribbons: " << ribbons.size() << "." << std::endl;
            
            // ai::saveVector("./open", Wk);
            // ai::saveVector("./open2", openingNew);
            Wk = openingNew;
            distances = distancesNew;
            ai::saveMatrix("./distances", distances);
            //T = 1000000;
